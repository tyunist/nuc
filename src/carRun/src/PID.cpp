...
/////////////////////////////////////////////////
double PID::Update(double _error, double circleTime)
{
  double pTerm, dTerm, iTerm;
  this->pErr = _error;

  if (circleTime == 0 || circleTime > 100 || circleTime < - 100)
    return 0.0;
  ...
  this->iErr = this->iErr + circleTime * this->pErr;
  ...
  if (circleTime != 0)
  {
    this->dErr = (this->pErr - this->pErrLast) / circleTime;
  ...
  if (cmdMax != 0.0 && this->cmd > this->cmdMax)
    this->cmd = this->cmdMax;
  if (cmdMax != 0.0 && this->cmd < this->cmdMin)
